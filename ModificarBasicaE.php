...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $sql = "SELECT TituloBE, Parrafo1BE, Parrafo2BE, Parrafo3BE, ImagenBE FROM basicae WHERE idBE = 1";  // Suponiendo que idBE = 1 es el único registro
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['img-basicae']) && $_FILES['img-basicae']['error'] == 0) {
        ...
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $imgPath = $row['ImagenBE'];  // Usamos la imagen existente
        } else {
            $imgPath = null;
        }
    }
